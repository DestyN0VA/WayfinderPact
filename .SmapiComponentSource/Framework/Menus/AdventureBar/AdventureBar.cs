...
namespace SwordAndSorcerySMAPI.Framework.Menus.AdventureBar
{
    ...
    internal class AdventureBar(bool editing) : IClickableMenu(0, (Game1.uiViewport.Height - 64 * 8 + 12 * 2) / 2, 64 * 2 + 12 * 2, 64 * 8 + 12 * 2)
    {
        ...
        public void TryPlace(ref Abilities.Ability abil, int x, int y)
        {
        ...
        public override void draw(SpriteBatch b)
        {
            ...
            drawTextureBox(b, Game1.menuTexture, new Rectangle(0, 256, 60, 60), xPositionOnScreen, yPositionOnScreen, width, height, Color.White, 1f, drawShadow: false);
            for (int ibar = 0; ibar < 2; ++ibar)
            {
                for (int islot = 0; islot < 8; ++islot)
                {
            ...
            if (!editing)
            {
                ...
                drawTextureBox(b, xPositionOnScreen, yPositionOnScreen + height - 12, width, 32 + 12 + 12, Color.White);
                float perc = 0;
            ...
            if (hover != null)
            {
                drawToolTip(b, hover.Description().Replace('^', '\n'), hover.Name(), null);
            }
        }
    }
}
