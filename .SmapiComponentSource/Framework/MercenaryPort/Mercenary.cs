...
namespace SwordAndSorcerySMAPI.Framework.MercenaryPort
{
    ...
    public class Mercenary : Character
    {
        ...
        public void UpdateForFarmer(Farmer farmer, int numInFormation, GameTime gameTime)
        {
            ...
            var atk = new MercenaryActionData()
            {
                Id = "MeleeAttack",
                ActionType = "MeleeAttackNearestMonster",
            };
            MercenaryData data = new();
            data.Actions.Add(atk);
            data.ActionsByPriority ??= new();
            data.ActionsByPriority.Add([atk]);
                ...
                if (targeting != null && attackWeapon != null)
                {
                    ...
                    if (Vector2.Distance(targeting.Position, Position) < Game1.tileSize * 1.25f)
                    {
                        if (!hasSwung)
                        {
                            ...
                            untilEndAction.Value = swingTime + 0.05f;
                        }
        ...
        {
            b.Draw(Sprite.Texture, getLocalPosition(Game1.viewport) + new Vector2(GetSpriteWidthForPositioning() * 4 / 2, GetBoundingBox().Height / 2 - 16), Sprite.SourceRect, Color.White * alpha, 0, new Vector2(Sprite.SpriteWidth / 2, Sprite.SpriteHeight * 3f / 4f), Math.Max(0.2f, Scale) * 4f, flip || Sprite.CurrentAnimation != null && Sprite.CurrentAnimation[Sprite.currentAnimationIndex].flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, drawOnTop ? 0.991f : StandingPixel.Y / 10000f));
            if (IsEmoting)
            {
                Vector2 emotePosition = getLocalPosition(Game1.viewport);
                emotePosition.Y -= 96f;
                b.Draw(Game1.emoteSpriteSheet, emotePosition, new Rectangle(CurrentEmoteIndex * 16 % Game1.emoteSpriteSheet.Width, CurrentEmoteIndex * 16 / Game1.emoteSpriteSheet.Width * 16, 16, 16), Color.White * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, StandingPixel.Y / 10000f);
            }
